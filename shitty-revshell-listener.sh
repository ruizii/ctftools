...
if [[ "$1" == "linux" ]]; then
    ...
    stty raw -echo && rcat listen -ie "/usr/bin/script -qc /bin/bash /dev/null; export TERM=xterm-256color; alias ls='ls --color=auto'; alias grep='grep --color=auto'; alias ip='ip --color=auto'" "$2" && reset
elif [[ "$1" == "windows" ]]; then
    echo -ne "\n"
